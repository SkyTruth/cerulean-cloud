...
def upgrade() -> None:
    ...
    # 2. Triggers (simplified – INSERT & DELETE shown; UPDATE handled likewise)
    op.execute("""
    ...
    SELECT
        ...
        'https://cerulean.skytruth.org/slicks/'||sl.id||'?ref=api&slick_id='||sl.id AS slick_url
    FROM slick             sl
    ...
    SELECT
        ...
        'https://cerulean.skytruth.org/slicks/'||sk.id||'?ref=api&slick_id='||sk.id AS slick_url,
    ...
    # 7. Incremental refresh helper
    op.execute("""
    CREATE OR REPLACE PROCEDURE refresh_slick_views() LANGUAGE plpgsql AS $$
    BEGIN
        -- Identify slick_ids that changed
        ...
        -- Refresh slick_plus rows
        REFRESH MATERIALIZED VIEW CONCURRENTLY slick_plus;

        -- Views depending on slick_plus need full refresh but are small
