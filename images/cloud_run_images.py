...
cloud_run_infer_image_url = registry_url.apply(
...
cloud_run_aoi_backfill_image_url = registry_url.apply(
    lambda url: f"{url}/{construct_name_images('cr-aoi-backfill-image')}"
)
...
model_weights = get_file_from_gcs(
    weights_bucket,
    weights_name,
    out_path="../cerulean_cloud/cloud_run_infer/model/model.pt",
)
cloud_run_infer_image = docker.Image(
...
cloud_run_aoi_backfill_image = docker.Image(
    construct_name_images("cr-aoi-backfill-image"),
    build=docker.DockerBuildArgs(
        context="../",
        dockerfile="../Dockerfiles/Dockerfile.cloud_run_aoi_backfill",
        target="final",
    ),
    image_name=cloud_run_aoi_backfill_image_url,
    registry=registry_info,
)
