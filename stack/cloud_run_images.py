...
cloud_run_aoi_backfill_image_url = (
    f"{artifact_registry_url}/{construct_name_images('cr-aoi-backfill-image')}:latest"
)
...
cloud_run_aoi_backfill_registry_image = docker.get_registry_image(
    name=cloud_run_aoi_backfill_image_url,
    opts=pulumi.InvokeOptions(provider=docker_provider),
)
...
cloud_run_aoi_backfill_image = docker.RemoteImage(
    construct_name_images("remote-aoi-backfill"),
    name=cloud_run_aoi_backfill_registry_image.name,
    pull_triggers=[cloud_run_aoi_backfill_registry_image.sha256_digest],
    opts=pulumi.ResourceOptions(provider=docker_provider),
)
