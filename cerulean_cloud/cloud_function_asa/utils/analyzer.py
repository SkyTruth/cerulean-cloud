...
class AISAnalyzer(SourceAnalyzer):
    ...
    def build_trajectories(self):
        ...
        For times between the first and last measurement, positions are generated by linear interpolation,
        while positions outside this interval are computed by Kalman filter extrapolation.
        """
        s1_time = self.s1_scene.start_time

        # Sort AIS data by timestamp and filter out already processed sources.
        ...
        for source_id, group in ais_data.groupby("ssvid"):
            ...
            # Filter display for points up to the scene start time.
            display_gdf = interp_gdf[interp_gdf.index <= s1_time].copy()

            # Format timestamps as ISO strings for GeoJSON output.
