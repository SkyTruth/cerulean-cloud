...
class AISAnalyzer(SourceAnalyzer):
    ...
    def check_data_availability(self):
        ...
        sql = """
            ...
            FROM `global-fishing-watch.pipe_ais_v5_published.segs_activity_daily`
...
class DarkAnalyzer(PointAnalyzer):
    ...
    def retrieve_sar_detection_data(self):
        ...
        filtered_matches AS (
        SELECT match.*
        FROM `global-fishing-watch.pipe_sar_v1_published.detect_scene_match_pipe_v5` AS match
        INNER JOIN scene_ids
            ON match.scene_id = scene_ids.scene_id
        WHERE match.score < .01 -- either no match or low confidence match
        -- Near-midnight detections can be in the following day's window.
        AND match.window_date BETWEEN DATE '{self.s1_scene.start_time.strftime("%Y-%m-%d")}'
            AND DATE_ADD(DATE '{self.s1_scene.end_time.strftime("%Y-%m-%d")}', INTERVAL 1 DAY)
        ),
    ...
    def check_data_availability(self):
        ...
        sql = f"""
            ...
            FROM `global-fishing-watch.pipe_sar_v1_published.detect_scene_match_pipe_v5`
            WHERE scene_id = '{self.s1_scene.scene_id}'
            -- Near-midnight detections can be in the following day's window.
            AND window_date BETWEEN DATE '{self.s1_scene.start_time.strftime("%Y-%m-%d")}'
                AND DATE_ADD(DATE '{self.s1_scene.end_time.strftime("%Y-%m-%d")}', INTERVAL 1 DAY);
        """
        df = pandas_gbq.read_gbq(
            sql,
