...
class SourceAnalyzer:
    ...
    def longest_path_tree(self, graph):
        """
    ...
    def find_longest_spine(self, cl):
        """
        ...
        for (
            linestring
        ) in cl.geometry.geoms:  # Use .geoms to access individual LineStrings
            coords = list(linestring.coords)
            for i in range(len(coords) - 1):
                count += 1
                graph.add_edge(coords[0], coords[i + 1])
        spine = self.longest_path_tree(graph)
        ...
        for component in connected_components:
            ...
            if len(subgraph.edges) > 0:
                ...
                )  # Add the end points of each edge
    ...
    def slick_to_curves(
        ...
        for _, row in slick_closed.iterrows():
            ...
            cl.geometry = self.find_longest_spine(cl)
            ...
            cl_geoms_list.append(cl.geometry)
        self.centerline_gdf = gpd.GeoDataFrame(
            {"geometry": cl_geoms_list}, crs=self.crs_meters
        ).to_crs(self.slick_gdf.crs)
...
class PointAnalyzer(SourceAnalyzer):
    ...
    def aggregate_extrema_and_area_fractions(
        ...
        extrema_list = []
        area_fractions_list = []
        # print("NUM OF POLYGONS", len(polygons), "WITH NUM_VERT", self.num_vertices)
    ...
    def calc_score_extremities_to_points(
        ...
        for i, neighbors in enumerate(extremity_indices):
            if neighbors:
                ...
                # print(len(all_extremity_points[neighbors]))
...
class DarkAnalyzer(PointAnalyzer):
    ...
    def __init__(self, s1_scene, **kwargs):
        ...
        self.centerline_gdf = kwargs.get("centerline_gdf", None)
