...
3. Add the following to source_to_tag (There's no way to have sqlacodegen output this relationship without a DB FK):
...
4. AoiUser keeps the child-table "geometry" column mapped as aoi_user_geometry.
...
class AoiType(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["read_perm"],
            ["permission.id"],
            name="fk_aoi_type_read_perm_permission",
        ),
    ...
    users: Mapped[Optional["Users"]] = relationship("Users")
...
class Aoi(Base):
    ...
    ext_id: Mapped[Optional[str]] = mapped_column(Text, server_default=text("NULL"))
...
class Slick(Base):
    ...
    cls: Mapped[Optional[int]] = mapped_column(Integer)
...
class SourceToTag(Base):
    ...
    source_ext: Mapped["Source"] = relationship(
        "Source",
        primaryjoin=lambda: and_(
            foreign(SourceToTag.source_ext_id) == Source.ext_id,
            foreign(SourceToTag.source_type) == Source.type,
